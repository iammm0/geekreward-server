...
type BountyService interface {
	CreateBounty(input dtos2.BountyDTO, userID uuid.UUID) (*tables2.Bounty, error)
	GetBounty(id uuid.UUID) (*tables2.Bounty, error)
	UpdateBounty(id uuid.UUID, input dtos2.BountyDTO) (*tables2.Bounty, error)
	...
	GetBountiesByUserID(userID uuid.UUID) ([]tables2.Bounty, error)
	GetReceivedBounties(userID uuid.UUID) ([]tables2.Bounty, error)
	GetUserBountyInteraction(userID, bountyID uuid.UUID) (*dtos2.BountyInteraction, error)
	...
	FindBounties(filters dtos2.BountyFilter) ([]tables2.Bounty, error)
	PostComment(userID, bountyID uuid.UUID, content string) (*tables2.Comment, error)
...
type bountyService struct {
	...
	applicationRepo  repositories2.ApplicationRepository
	notificationRepo repositories2.NotificationRepository
	milestoneRepo    repositories2.MilestoneRepository
}

// PostComment 用户对某个bounty发表评论
func (s *bountyService) PostComment(userID, bountyID uuid.UUID, content string) (*tables2.Comment, error) {
	// 构造 Comment 对象
	comment := &tables2.Comment{
		UserID:   userID,
		BountyID: bountyID,
		Content:  content,
	}
...
func (s *bountyService) GetCommentsByBountyID(bountyID uuid.UUID) ([]tables2.Comment, error) {
...
func (s *bountyService) CancelSettlementByPublisher(bountyID, userID uuid.UUID) error {
	...
	if bounty.Status != tables2.BountyStatusSettling {
	...
	// 状态改为Cancelled
	bounty.Status = tables2.BountyStatusCancelled
...
func (s *bountyService) FindBounties(filters dtos2.BountyFilter) ([]tables2.Bounty, error) {
...
func (s *bountyService) ConfirmMilestones(bountyID uuid.UUID, userID uuid.UUID) error {
	...
	if bounty.Status == tables2.BountyStatusCreated {
		bounty.Status = tables2.BountyStatusMilestonesConfirmed
		bounty.UpdatedAt = time.Now()
		if err := s.bountyRepo.UpdateBounty(bounty); err != nil {
			return err
		}
...
func (s *bountyService) ApplySettlement(bountyID, userID uuid.UUID) error {
	...
	if bounty.Status == tables2.BountyStatusMilestonesVerified {
		// 进入清算状态
		bounty.Status = tables2.BountyStatusSettling
		bounty.UpdatedAt = time.Now()
		if err := s.bountyRepo.UpdateBounty(bounty); err != nil {
			return err
		}
...
func (s *bountyService) CreateBounty(input dtos2.BountyDTO, userID uuid.UUID) (*tables2.Bounty, error) {
	deadline, err := time.Parse("2006-01-02", input.Deadline)
	if err != nil {
		log.Printf("Error parsing deadline: %v", err)
		return nil, err
	}
	bounty := &tables2.Bounty{
		Title:                   input.Title,
...
func (s *bountyService) GetBounty(id uuid.UUID) (*tables2.Bounty, error) {
	return s.bountyRepo.FindBountyByID(id)
}

// UpdateBounty 更新指定 ID 的悬赏令
func (s *bountyService) UpdateBounty(id uuid.UUID, input dtos2.BountyDTO) (*tables2.Bounty, error) {
...
func (s *bountyService) LikeBounty(userID, bountyID uuid.UUID) error {
	like := &tables2.Like{UserID: userID, BountyID: bountyID}
...
func (s *bountyService) RateBounty(userID, bountyID uuid.UUID, score float64) error {
	...
	if rating.BountyID != uuid.Nil {
		...
		// 添加新评分
		newRating := &tables2.Rating{UserID: userID, BountyID: bountyID, Score: score}
...
func (s *bountyService) GetBountiesByUserID(userID uuid.UUID) ([]tables2.Bounty, error) {
	return s.bountyRepo.FindByUserID(userID)
}

// GetReceivedBounties 获取用户接收的所有悬赏令
func (s *bountyService) GetReceivedBounties(userID uuid.UUID) ([]tables2.Bounty, error) {
	return s.bountyRepo.FindReceivedByUserID(userID)
}

// GetUserBountyInteraction 获取用户在指定悬赏令上的互动信息
func (s *bountyService) GetUserBountyInteraction(userID, bountyID uuid.UUID) (*dtos2.BountyInteraction, error) {
	liked, err := s.bountyRepo.IsBountyLikedByUser(userID, bountyID)
	if err != nil {
		return nil, err
	}
	...
	return &dtos2.BountyInteraction{Liked: liked, Score: score}, nil
}
...
func (s *bountyService) SettleBountyAccounts(bountyID uuid.UUID) error {
	...
	for _, app := range approvedApplications {
		...
		// 发送通知给申请者
		notification := &tables2.Notification{
			UserID:      app.UserID,
			Title:       "Bounty Settle",
	...
	// 更新状态为Settled
	bounty.Status = tables2.BountyStatusSettled
	bounty.UpdatedAt = time.Now()
	if err := s.bountyRepo.UpdateBounty(bounty); err != nil {
		return err
	}
