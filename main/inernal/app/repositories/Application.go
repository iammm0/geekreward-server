...
type ApplicationRepository interface {
	Create(application *tables2.Application) error
	FindAllByBountyID(bountyID uuid.UUID) ([]tables2.Application, error)
	UpdateApplicationStatus(applicationID uuid.UUID, status string) error
	GetApprovedApplicationsByBountyID(bountyID uuid.UUID) ([]*tables2.Application, error)
	...
	FindByID(applicationID uuid.UUID) (*tables2.Application, error)
}
...
func (r *applicationRepository) FindByID(applicationID uuid.UUID) (*tables2.Application, error) {
	var app tables2.Application
...
func (r *applicationRepository) ApproveApplication(applicationID uuid.UUID, receiverID uuid.UUID) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		// 1. 更新申请状态为 "approved"
		if err := tx.Model(&tables2.Application{}).
		...
		if err := tx.Model(&tables2.Bounty{}).
...
func (r *applicationRepository) HasUserApplied(bountyID uuid.UUID, UserID uuid.UUID) (bool, error) {
	...
	err := r.db.Model(&tables2.Application{}).
...
func (r *applicationRepository) Create(application *tables2.Application) error {
	return r.db.Create(application).Error
}

func (r *applicationRepository) FindAllByBountyID(bountyID uuid.UUID) ([]tables2.Application, error) {
	var apps []tables2.Application
...
func (r *applicationRepository) UpdateApplicationStatus(applicationID uuid.UUID, status string) error {
	return r.db.Model(&tables2.Application{}).
		Where("id = ?", applicationID).
		Update("status", status).Error
}

func (r *applicationRepository) GetApprovedApplicationsByBountyID(bountyID uuid.UUID) ([]*tables2.Application, error) {
	var apps []*tables2.Application
